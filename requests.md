...
==================
 ...
 Data Collector's main goal is to provide methods to search data collected from Resources.

This service is used both by applications and the Catalog Service, and offers developers access to information coming from different sensors scattered throughout the city. For example, with this service one could obtain data of all temperature sensors on USP.


Requests to this service usually involve receiving or sending a data structure in JSON format. Check out the example below:
...
The fields present on the above example may vary according to the request.
...
*     **uuid**: A non-quoted String corresponding to the resource's uuid (not to be confused with the local id assigned by the Data Collector!). May have been upplied beforehand by the Discovery, or stored directly by the client's application.
*     **capabilities**: Capabilities are capacity that resources are able to respond to which we keep the data.
*     **value**:  The collected value. It can be an number (float,integer) or string.
*     **date**: This field respresent the date when the data was collected.

<br>

1.  Getting data from all resources  (Implemented)
...
Returns all the Data from all resources. Every Data has an array of Details; each Detail contains the value collected, the capability (such as “temperature”, “luminosity”, ...) and date.
...
2.  Getting data from a specific resource or set of resources  (Implemented)
...
Returns all information data of an specified resources.
...
3. Getting last data from a specific resource or set of resources  (Implemented)
...
4. Getting the last data from all resources  (Implemented)
