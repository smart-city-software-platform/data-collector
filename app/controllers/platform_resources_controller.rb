	...
	def create
		...
		begin
			if platform_resource.save
				...
				# Started worker
puts '-' * 30
				x = WorkerSupervisor.instance
puts x.resource_status platform_resource.id
				x.set_resource_collector_status(platform_resource.id,
												WorkerSupervisor::ACTIVE)
puts x.resource_status platform_resource.id
puts '-' * 30
				CollectData.perform_async(platform_resource.uri, platform_resource.id,
